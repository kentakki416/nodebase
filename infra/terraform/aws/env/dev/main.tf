...
module "vpc" {
  ...
  security_group_rules = [
    # ALB Ingress - インターネットからHTTPS を受け付ける
    {
    ...
    # ALB Ingress - HTTP は当面 SG 上は開けておく (将来 HTTP→HTTPS リダイレクト listener を追加するときに使う)
    {
      security_group_name = "alb"
      type                = "ingress"
      from_port           = 80
...
# - DATABASE_URL は apply 後に手動で /nodebase-dev/app secret に追加する
...
# =============================================================================
# ACM 証明書 (*.dev.nodebase.com)
# =============================================================================
# - prd の zone (nodebase.com) 内で DNS 検証する
...
module "alb" {
  ...
  # === HTTPS 化 ===
  # ACM ワイルドカード (*.dev.nodebase.com) を HTTPS listener (443) にアタッチ
  enable_https    = true
...
# =============================================================================
# Route53 A レコード (api.dev.nodebase.com → ALB)
# =============================================================================
...
#   (nodebase-migration) + 専用 Dockerfile (apps/api/Dockerfile.migration) を使う
